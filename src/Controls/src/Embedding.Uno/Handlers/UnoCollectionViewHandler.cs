...
/// <c>ListViewBase</c>, as an experimental alternative for Uno targets.
/// </summary>
/// <remarks>
/// <para>
/// This alternative was introduced for failures in earlier ListViewBase-based runtime combinations.
/// Compare it with the current default handlers before opting in: this replacement has a smaller API
/// surface. ItemsRepeater provides a layout and element factory without a control template.
/// </para>
/// <para>
...
public partial class UnoCollectionViewHandler : ViewHandler<ReorderableItemsView, PlatformView>
{
	...
	void OnScrollToRequested(object? sender, ScrollToRequestEventArgs args)
	{
		...
		{
			throw new InvalidOperationException("The collection handler is not connected.");
		}
		if (VirtualView.IsGrouped)
		{
			throw new NotSupportedException("Grouped scrolling is not supported by this handler.");
		}
		var index = args.Mode == ScrollToMode.Position
			? args.Index
			: ItemSourceIndex.Find(_repeater.ItemsSourceView, args.Item);
		if (index < 0 || index >= (_repeater.ItemsSourceView?.Count ?? 0))
		{
			throw new ArgumentOutOfRangeException(nameof(args), "The requested item is not in the collection.");
		}
		var horizontal = VirtualView.ItemsLayout is ItemsLayout { Orientation: ItemsLayoutOrientation.Horizontal };
		var element = (PlatformView)_repeater.GetOrCreateElement(index);
		// The newly realized anchor needs layout before its target rectangle is available.
		_repeater.UpdateLayout();
		var bounds = element.TransformToVisual(_repeater).TransformBounds(
			new global::Windows.Foundation.Rect(0, 0, element.ActualWidth, element.ActualHeight));
		var start = horizontal ? bounds.X + _repeater.Margin.Left : bounds.Y + _repeater.Margin.Top;
		var size = horizontal ? bounds.Width : bounds.Height;
		var current = horizontal ? _scrollViewer.HorizontalOffset : _scrollViewer.VerticalOffset;
		var viewport = horizontal ? _scrollViewer.ViewportWidth : _scrollViewer.ViewportHeight;
		var maximum = horizontal ? _scrollViewer.ScrollableWidth : _scrollViewer.ScrollableHeight;
		var offset = args.ScrollToPosition switch
		{
			ScrollToPosition.Start => start,
			ScrollToPosition.Center => start + (size - viewport) / 2,
			ScrollToPosition.End => start + size - viewport,
			_ => start < current ? start : start + size > current + viewport ? start + size - viewport : current
		};
		offset = Math.Clamp(offset, 0, maximum);
		_scrollViewer.ChangeView(horizontal ? offset : null, horizontal ? null : offset, null, disableAnimation: !args.IsAnimated);
	}
