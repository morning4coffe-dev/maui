...
/// <summary>
/// An experimental <see cref="CarouselView"/> handler built on <c>ItemsRepeater</c>.
/// Compare its behavior with the current default handlers before opting in.
/// </summary>
/// <remarks>
/// <para>
...
public partial class UnoCarouselViewHandler : ViewHandler<CarouselView, PlatformView>
{
	...
	Microsoft.UI.Xaml.Controls.ItemsSourceView? _observedItems;
	bool _needsSourceRefresh;
	...
	int InnerCount
	{
		get
		{
			...
			return _repeater?.ItemsSourceView?.Count ?? 0;
		}
	}
	...
	protected override PlatformView CreatePlatformView()
	{
		...
		_repeater.LayoutUpdated += OnItemsLayoutUpdated;
	...
	protected override void DisconnectHandler(PlatformView platformView)
	{
		...
		_needsSourceRefresh = false;
		VirtualView.VisibleViews.Clear();
	...
	{
		if (carousel.Position >= 0 && carousel.Position < handler.InnerCount &&
			Equals(handler.ItemAt(carousel.Position), carousel.CurrentItem))
		{
			handler.ScrollToRealIndex(carousel.Position, carousel.IsScrollAnimated);
			return;
		}
		var index = ItemSourceIndex.Find(handler._repeater?.ItemsSourceView, carousel.CurrentItem);
		if (index >= 0)
		{
			index = handler._loopingSource?.ToInnerIndex(index) ?? index;
			if (carousel.Position != index)
			{
				carousel.Position = index;
			}
			handler.ScrollToRealIndex(index, carousel.IsScrollAnimated);
		}
	}
	...
	void UpdateItemsSource(CarouselView carousel)
	{
		...
		_observedItems = _repeater.ItemsSourceView;
		if (_observedItems is not null)
		{
			_observedItems.CollectionChanged += OnItemsChanged;
		}
		_needsSourceRefresh = true;
	...
	void OnItemsChanged(object? sender, NotifyCollectionChangedEventArgs args) => _needsSourceRefresh = true;

	void OnItemsLayoutUpdated(object? sender, object args)
	{
		if (!_needsSourceRefresh || VirtualView is null || _scrollViewer?.IsLoaded != true || ItemStep <= 0)
		{
			return;
		}
		_needsSourceRefresh = false;
		var position = Math.Clamp(VirtualView.Position, 0, Math.Max(0, InnerCount - 1));
		ReportPosition(position);
		ScrollToRealIndex(position, animate: false);
		UpdateVisibleViews();
	}
	...
	{
		var source = _repeater?.ItemsSourceView;
		return source is not null && index >= 0 && index < source.Count ? source.GetAt(index) : null;
	}
}
