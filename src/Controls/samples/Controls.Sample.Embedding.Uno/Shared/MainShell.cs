...
internal sealed class MainShell : UserControl
{
	...
	async Task RunProductionProbeAsync()
	{
		try
		{
			Console.WriteLine($"PRODUCTION-MODE {MauiProgram.HandlerMode}");
			var result = await ProductionCompatibilityProbe.RunAsync(_session, _secondHost);
			_probeResults.Text = result.Report;
			Console.WriteLine(result.Report);
			_session.PlatformWindow.Title = result.Passed ? "PRODUCTION-PROBE PASS" : "PRODUCTION-PROBE FAIL";
		}
		catch (Exception error)
		{
			Console.WriteLine(error);
			_session.PlatformWindow.Title = "PRODUCTION-PROBE FAIL";
		}
	}
