...
This is a source-mode experiment, not a production-ready consumer package. Use
the renderer maintenance guide's exact runtime/dependency qualifications.
...
The code-only application root also registers the Lottie visual-source provider
before creating `XamlControlsResources`. Keep the Lottie, Graphics2DSK and Skottie
references together; otherwise adding a MAUI `ActivityIndicator` can produce
an unsupported-control placeholder instead of an animated indicator.
...
A static application must complete its splash lifecycle without a click,
resize or forced repaint. A loader that remains above initialized content is
a startup defect. The combined runtime includes the separate Uno frame-request
correction; that dependency must be present in a release.
